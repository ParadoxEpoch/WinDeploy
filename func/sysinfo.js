...
const getSystemParam = {
    ...
    disks: async () => {
        ...
        const diskInfo = await PowerShell.$`Get-CimInstance -ClassName Win32_DiskDrive | Select @{Name="id";Expression={($_.DeviceID) -replace '\\\\\\\\.\\\\PHYSICALDRIVE', ''}},model,@{Name="serial";Expression={$_.SerialNumber}},partitions,@{Name="sizeGB";Expression={[Math]::Truncate($_.Size / 1073741824)}} | ConvertTo-JSON`;
        const mountInfo = await PowerShell.$`Get-CimInstance -Class win32_logicaldisktopartition | Select @{Name="mount";Expression={(($_.Dependent) -replace 'Win32_LogicalDisk \\(DeviceID = "', '') -replace ':"\\)', ''}},@{Name="disk";Expression={(($_.Antecedent) -replace '^.+Disk #', '') -replace ',.+', ''}},@{Name="partition";Expression={(($_.Antecedent) -replace '^.+Partition #', '') -replace '".+', ''}} | ConvertTo-JSON`;
